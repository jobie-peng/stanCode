...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    d_baby = {}
    # Check if the name already exists in dictionary
    # Not exists -> Add name and data
    # Exists -> Check the year and rank and amend data accordingly
    if name not in name_data:
        d_baby[year] = rank
        name_data[name] = d_baby
    else:
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        for lines in f:
            # First line contains year data
            if year_data:
                year = lines.strip()
                year_data = False
            # All other lines contain rank, name1, name2
            else:
                tokens = lines.split(',')
                rank = tokens[0].strip()
