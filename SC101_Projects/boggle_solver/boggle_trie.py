...
This file put the dictionary in a TrieNode and includes the prefix check function.
"""
...
def search_word(boggle, words_list, i, j, current_s, found_words):
	...
	if len(current_s) >= 2 and not words_list.startsWith(current_s):
		return

	# Choose
	current_s += boggle[i][j]
	if len(current_s) >= 4 and words_list.search(current_s) and current_s not in found_words:
